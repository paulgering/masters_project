...
p <- ggplot(data = my_table1, aes(x = reorder(variable, IQR_above_median),
  ...
  geom_pointrange() + 
  coord_flip() + 
  labs(x = NULL, y = "Change in vocabulary size per 1 SD increase from the median in the predictor") + theme(axis.title.x = element_text(size = 10, margin = margin(t = 15)), plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "inches"))
