...
Conducting a prospective power analysis to determine the sample size that would be required to detect a small/medium effect size. The median of 
...
```{r}
M0 = glm(18m_cdi_vocab_count ~ condition + age, family = Poisson, data=df3)
```

### Model 1 - Count of infant 'intentional' Prelinguistic Communicative Acts

```{r}
M1 = glm(18m_cdi_vocab_count ~ condition + age + 12m_pca_count ,family=poisson,data=df3)
...
```{r}
M2 = glm(18m_cdi_vocab_count ~ condition + age + 12m_responses_pca_only_count ,family=poisson,data=df3)
...
```{r}
M3 = glm(18m_cdi_vocab_count ~ condition + age + 12m_cl_pca_only_count ,family=poisson,data=df3)
...
```{r}
M4 = glm(18m_cdi_vocab_count ~ condition + age + 12m_sc_pca_only_count ,family=poisson,data=df3)
...
```{r}
M5 = glm(18m_cdi_vocab_count ~ condition + age + 12m_responses_all_count ,family=poisson,data=df3)
...
```{r}
M6 = glm(18m_cdi_vocab_count ~ condition + age + 12m_cl_all_count ,family=poisson,data=df3)
...
```{r}
M7 = glm(18m_cdi_vocab_count ~ condition + age + 12m_sc_all_count ,family=poisson,data=df3)
...
```{r}
M1_nb = glm.nb(18m_cdi_vocab_count ~ condition + age + 12m_pca_count ,family=poisson,data=df3)

summary(M1)
```

```{r}
M2_nb = glm.nb(18m_cdi_vocab_count ~ condition + age + 12m_responses_pca_only_count ,family=poisson,data=df3)

summary(M2)
```

```{r}
M3_nb = glm.nb(18m_cdi_vocab_count ~ condition + age + 12m_cl_pca_only_count ,family=poisson,data=df3)

summary(M3)
```

```{r}
M4_nb = glm.nb(18m_cdi_vocab_count ~ condition + age + 12m_sc_pca_only_count ,family=poisson,data=df3)
...
```{r}
M5_nb = glm.nb(18m_cdi_vocab_count ~ condition + age + 12m_responses_all_count ,family=poisson,data=df3)

summary(M5)
```

```{r}
M6_nb = glm.nb(18m_cdi_vocab_count ~ condition + age + 12m_cl_all_count ,family=poisson,data=df3)

summary(M6)
```

```{r}
M7_nb = glm.nb(18m_cdi_vocab_count ~ condition + age + 12m_sc_all_count ,family=poisson,data=df3)
...
EXAMPLE: If overdispersion is detected for model 1 and the null model: 

```{r}
anova(M1_nb, M0_nb)
```

EXAMPLE: If overdispersion is detected for model 1 but not the null model: 

```{r}
anova(M1_nb, M0)
```

EXAMPLE: If overdispersion is detected for the null model but not model 1: 

```{r}
anova(M1, M0_nb)
```

## Comparing the AIC of the models to test predictions 2, 3, and 4 and to answer two questions in a exploratory analyses
