...
library(renv) #Store packages and keep account of package versions
renv::restore()
...
```{r}
M0 = glm(sev_cdi_vocab_count ~ condition + age_at_survey, family = poisson, data=df2)
```

### Model 1 - Count of infant 'intentional' Prelinguistic Communicative Acts

```{r}
M1 = glm(sev_cdi_vocab_count ~ condition + age_at_survey + twe_pca_count ,family=poisson,data=df2)
...
```{r}
M2 = glm(sev_cdi_vocab_count ~ condition + age_at_survey + twe_responses_pca_only_count ,family=poisson,data=df2)
...
```{r}
M3 = glm(sev_cdi_vocab_count ~ condition + age_at_survey + twe_cl_pca_only_count ,family=poisson,data=df2)
...
```{r}
M4 = glm(sev_cdi_vocab_count ~ condition + age_at_survey + twe_sc_pca_only_count ,family=poisson,data=df2)
...
```{r}
M5 = glm(sev_cdi_vocab_count ~ condition + age_at_survey + twe_responses_all_count ,family=poisson,data=df2)
...
```{r}
M6 = glm(sev_cdi_vocab_count ~ condition + age_at_survey + twe_cl_all_count ,family=poisson,data=df2)
...
```{r}
M7 = glm(sev_cdi_vocab_count ~ condition + age_at_survey + twe_sc_all_count ,family=poisson,data=df2)
...
```{r}
M0_nb = glm.nb(sev_cdi_vocab_count ~ condition + age_at_survey,data=df2)

summary(M0_nb)
```

```{r}
M1_nb = glm.nb(sev_cdi_vocab_count ~ condition + age_at_survey + twe_pca_count,data=df2)

summary(M1_nb)
```

```{r}
M2_nb = glm.nb(sev_cdi_vocab_count ~ condition + age_at_survey + twe_responses_pca_only_count,data=df2)

summary(M2_nb)
```

```{r}
M3_nb = glm.nb(sev_cdi_vocab_count ~ condition + age_at_survey + twe_cl_pca_only_count,data=df2)

summary(M3_nb)
```

```{r}
M4_nb = glm.nb(sev_cdi_vocab_count ~ condition + age_at_survey + twe_sc_pca_only_count,data=df2)
...
```{r}
M5_nb = glm.nb(sev_cdi_vocab_count ~ condition + age_at_survey + twe_responses_all_count,data=df2)

summary(M5_nb)
```

```{r}
M6_nb = glm.nb(sev_cdi_vocab_count ~ condition + age_at_survey + twe_cl_all_count,data=df2)

summary(M6_nb)
```

```{r}
M7_nb = glm.nb(sev_cdi_vocab_count ~ condition + age_at_survey + twe_sc_all_count,data=df2)
